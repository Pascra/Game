    ...
    if(result == NULL)
	{
        ...
        */
        ...
        for (const auto& mapLayer : mapData.layers) {
            ...
            if (mapLayer->properties.GetProperty("Colissions") != NULL && mapLayer->properties.GetProperty("Colissions")->value == true) {
                ...
                for (int i = 0; i < mapData.width; i++) {
                    for (int j = 0; j < mapData.height; j++) {
                        ...
                        int gid = mapLayer->Get(i, j);
                        // Check if the gid is different from 0 - some tiles are empty
                        if (gid == 2881) { // gid that corresponds to collision layer
                            Vector2D mapCoord = MapToWorld(i, j);
                            PhysBody* platform = Engine::GetInstance().physics.get()->CreateRectangle(
                                mapCoord.getX() + mapData.tileWidth / 2,
                                mapCoord.getY() + mapData.tileHeight / 2,
                                mapData.tileWidth,
                                mapData.tileHeight, STATIC
                            );
...
{
    bool ret = true;
    for (pugi::xml_node propertieNode = node.child("properties").child("property"); propertieNode != NULL; propertieNode = propertieNode.next_sibling("properties")) {
